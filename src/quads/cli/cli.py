...
class QuadsCli:
                ...
                if cloud:
                    try:
                        self.quads.update_cloud(
                            cloud.name, {"last_redefined": datetime.now().isoformat().split(".")[0][:-3]}
                        )
                    except (APIServerException, APIBadRequest) as ex:  # pragma: no cover
                        raise CliException(str(ex))
            elif assignment:
    ...
    def action_regen_instack(self):  # pragma: no cover
        regen_instack()
    ...
    def action_regen_wiki(self):  # pragma: no cover
    ...
    def action_notify(self):  # pragma: no cover
        notify(self.logger)
