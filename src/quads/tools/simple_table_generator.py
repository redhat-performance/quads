...
class HostGenerate:
    ...
    async def get_current_host_schedules(self, host_name):
        if host_name not in self.current_host_schedules:
            current_host_schedules = await self.fetch_schedule(payload={"host": host_name})
            self.current_host_schedules[host_name] = current_host_schedules
        else:
            current_host_schedules = self.current_host_schedules[host_name]
        return current_host_schedules
