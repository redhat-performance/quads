...
quads_async = QuadsApiAsync(Config)
all_samples = []
...
async def fetch_schedule(payload):
    schedules = await quads_async.async_get_current_schedules(payload)
    if schedules:
...
async def process_hosts(host, _days, _month, _year):
    ...
    for j in range(1, _days + 1):
        ...
        tasks.append(fetch_schedule(payload))
...
async def generator(_host_file, _days, _month, _year, _gentime):
    if _host_file:
        with open(_host_file, "r") as f:
            reader = csv.reader(f)
            hosts = list(reader)
    else:
        filtered_hosts = await quads_async.async_filter_hosts(data={"retired": False, "broken": False})
        hosts = sorted(filtered_hosts, key=lambda x: x.name)

    lines = []
    __days = []
    non_allocated_count = 0
    tasks = [process_hosts(host, _days, _month, _year) for host in hosts]
    results = await asyncio.gather(*tasks)
    ...
    total_current_schedules = await quads_async.async_get_current_schedules()
...
if __name__ == "__main__":
    ...
    asyncio.run(generator(host_file, days, month, year, gentime))
