...
class Badfish:
    ...
    async def find_os_deployment_resource(self):
        ...
        candidates = [
            "%s%s/Oem/Dell/DellOSDeploymentService" % (self.host_uri, self.system_resource),
            "%s/Dell/Systems/%s/DellOSDeploymentService" % (self.root_uri, system_id),
        ]

        for candidate in candidates:
            _response = await self.get_request(candidate)
            if _response.status == 200:
    ...
    async def check_remote_image(self):
        if not await self.check_os_deployment_support():
            return False
        _uri = "%s/Actions/DellOSDeploymentService.GetAttachStatus" % self.os_deployment_resource
        _headers = {"Content-Type": "application/json"}
        _response = await self.post_request(_uri, payload={}, headers=_headers)
        try:
            raw = await _response.text("utf-8", "ignore")
    ...
    async def boot_remote_image(self, nfs_path):
        if not await self.check_os_deployment_support():
            return False
        _uri = "%s/Actions/DellOSDeploymentService.BootToNetworkISO" % self.os_deployment_resource
        _headers = {"Content-Type": "application/json"}
        try:
    ...
    async def detach_remote_image(self):
        if not await self.check_os_deployment_support():
            return False
        _uri = "%s/Actions/DellOSDeploymentService.DetachISOImage" % self.os_deployment_resource
        _headers = {"Content-Type": "application/json"}
        _response = await self.post_request(_uri, payload={}, headers=_headers)
        if _response.status == 200:
