...
def get_dynamic_navigation():
    ...
    for file in files:
        ...
        if file.endswith(".html"):
            link['href'] = f"content/{file.replace('.html','')}"
    ...
    for sub in submenus:
        ...
        sub_path = os.listdir(os.path.join(WEB_CONTENT_PATH, sub))
        html_links = [file for file in sub_path if file.name.endswith(".html")]
        for hl in html_links:
            link['href'] = f"content/{hl.replace(".html","")}"
            link['text'] = hl.replace(".html","").replace("_"," ")
            sub_links.append(link)
        
        direct_links = [file for file in sub_path if not file.endswith(".html")]
...
def get_file_paths():
    exclude_dirs = ['.git','static']
    file_paths = []
    for (root,dirs,files) in os.walk(WEB_CONTENT_PATH):
