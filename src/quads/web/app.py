...
from quads.web.controller.static_content import StaticContent
...
if Config["github_repo"]:
    StaticContent().download_static_content(repo_link=Config["github_repo"])
quads = Quads(Config)
...
@flask_app.route("/r640")
def create_r640():
    r640_list = []
    if os.path.exists('./static/files/r640'):
        with open('./static/files/r640') as r640:
            for row in r640.readlines():
                r640_list.append(row)
    return render_template("wiki/r640.html", r640_list=r640_list)


@flask_app.route("/r650")
def create_r650():
    r650_list = []
    if os.path.exists("./static/files/r650"):
        with open('./static/files/r650') as r640:
            for row in r640.readlines():
                r650_list.append(row)
    return render_template("wiki/r650.html", r650_list=r650_list)


@flask_app.route("/1029u")
def create_1029u():
    u1029_list = []
    if os.path.exists('./static/files/1029u'):
        with open('./static/files/1029u') as r640:
            for row in r640.readlines():
                u1029_list.append(row)
    return render_template("wiki/1029u.html", u1029_list=u1029_list)


@flask_app.route("/fc640")
def create_fc640():
    return render_template("wiki/fc640.html")


@flask_app.route("/mellanox")
def create_mellanox():
    mellanox = []
    if os.path.exists("./static/files/mellanox"):
        with open('./static/files/mellanox') as file:
            for row in file.readlines():
                mellanox.append(row)
    return render_template("wiki/mellanox.html", mellanox=mellanox)


@flask_app.route("/mellanox-cx6")
def create_mellanox_cx6():
    mellanox_cx6 = []
    if os.path.exists("./static/files/mellanox-cx6"):
        with open('./static/files/mellanox_cx6') as file:
            for row in file.readlines():
                mellanox_cx6.append(row)
    return render_template("wiki/mellanox_cx6.html", mellanox_cx6=mellanox_cx6)


@flask_app.route("/public-vlans")
def create_public_vlans():
    return render_template("wiki/public_vlans.html")
