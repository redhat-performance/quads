...
if ( isset( $form_unserialized_meta_value['controls'] ) && count( $form_unserialized_meta_value['controls'] ) > 0 ) {
	/**
	 *
	 * This function is used to get currency symbol.
	 *
			...
			if ( isset( $form_unserialized_meta_value ) && isset( $form_unserialized_meta_value['controls'] ) ) {
				if ( count( $form_unserialized_meta_value['controls'] ) > 0 ) {
					foreach ( $form_unserialized_meta_value['controls'] as $values ) {
						...
						$input_class_right_control       = 'right' === $label_placement ? "right-placement-input-contact-bank_$random" : '';
						...
						switch ( $control_name ) {
							...
							case 'radio-list':
								...
								break;
							case 'html':
								$input_class   = "class='form-group default_text $element_class'";
								$control_class = "class='sub_div $container_class'";
								break;
							case 'email':
								$input_validation_type = '';
							...
							<div style="clear:both;"<?php echo isset( $control_class ) ? $control_class : ''; // WPCS: XSS ok. ?> name="ux_sub_div_<?php echo $timestamp; // WPCS: XSS ok. ?>" id="ux_sub_div_<?php echo $timestamp; // WPCS: XSS ok. ?>">
									...
									if ( 'paragraph' === $control_name ) {
										...
										?>
										<select style="<?php echo $input_style_class; // WPCS: XSS ok. ?>" name="ux_txt_singal_line_text_<?php echo $timestamp; // WPCS: XSS ok. ?>" id="ux_txt_singal_line_text_<?php echo $timestamp; // WPCS: XSS ok. ?>" <?php echo $input_class; // WPCS: XSS ok. ?> type="text" autocomplete="<?php echo isset( $values['autocomplete_type'] ) && esc_attr( $values['autocomplete_type'] ) === 'enable' ? 'off' : 'on'; ?>" <?php echo isset( $values['disable_input'] ) && esc_attr( $values['disable_input'] ) === 'enable' ? 'disabled=disabled' : ''; ?>>
											<?php
										...
										?>
										<input style="<?php echo $input_style_class; // WPCS: XSS ok. ?>" <?php echo $onfocus_event; // WPCS: XSS ok. ?> placeholder="<?php echo isset( $values['placeholder'] ) ? esc_attr( $values['placeholder'] ) : ''; ?>" name="ux_txt_singal_line_text_<?php echo $timestamp; // WPCS: XSS ok. ?>" <?php echo $input_validation_type; // WPCS: XSS ok. ?> id="ux_txt_singal_line_text_<?php echo $timestamp; // WPCS: XSS ok. ?>" <?php echo $input_class; // WPCS: XSS ok. ?> type="email" value="<?php echo isset( $values['default_value'] ) ? esc_attr( $values['default_value'] ) : ''; ?>" autocomplete="<?php echo isset( $values['autocomplete_type'] ) && 'enable' === esc_attr( $values['autocomplete_type'] ) ? 'off' : 'on'; ?>" <?php echo isset( $values['disable_input'] ) && esc_attr( $values['disable_input'] ) === 'enable' ? 'disabled=disabled' : ''; ?> <?php echo $onkeyup_event; // WPCS: XSS ok. ?>>
										<?php
									} elseif ( 'checkbox' === $control_name ) {
										?>
										<input type="checkbox"  class="checkbox_class" name="ux_txt_singal_line_text_<?php echo $timestamp; // WPCS: XSS ok. ?>" <?php echo $input_validation_type; // WPCS: XSS ok. ?> id="ux_txt_singal_line_text_<?php echo $timestamp; // WPCS: XSS ok. ?>" <?php echo $input_class; // WPCS: XSS ok. ?> type="text"  autocomplete="<?php echo isset( $values['autocomplete_type'] ) && esc_attr( $values['autocomplete_type'] ) === 'enable' ? 'off' : 'on'; ?>" <?php echo isset( $values['disable_input'] ) && esc_attr( $values['disable_input'] ) === 'enable' ? 'disabled=disabled' : ''; ?>><?php echo isset( $values['default_value'] ) ? esc_attr( $values['default_value'] ) : ''; ?>
										<?php
										...
										?>
										<input style="<?php echo $input_style_class; // WPCS: XSS ok. ?>" <?php echo $onfocus_event; // WPCS: XSS ok. ?> placeholder="<?php echo isset( $values['placeholder'] ) ? esc_attr( $values['placeholder'] ) : ''; ?>" name="ux_txt_singal_line_text_<?php echo $timestamp; // WPCS: XSS ok. ?>" <?php echo $input_validation_type; // WPCS: XSS ok. ?> id="ux_txt_singal_line_text_<?php echo $timestamp; // WPCS: XSS ok. ?>" <?php echo $input_class; // WPCS: XSS ok. ?> type="text" value="<?php echo isset( $values['default_value'] ) ? esc_attr( $values['default_value'] ) : ''; ?>" autocomplete="<?php echo isset( $values['autocomplete_type'] ) && esc_attr( $values['autocomplete_type'] ) === 'enable' ? 'off' : 'on'; ?>" <?php echo isset( $values['disable_input'] ) && esc_attr( $values['disable_input'] ) === 'enable' ? 'disabled=disabled' : ''; ?> <?php echo $onkeyup_event; // WPCS: XSS ok. ?>>
										<?php
									}
									?>
