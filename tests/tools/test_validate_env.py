...
@pytest.fixture
def validate_env_fixture(request):
    ...
    InterfaceDao.create_interface(
        HOST1, IFNAME1, IFBIOSID1, IFMAC1, IFIP1, IFPORT1, IFSPEED, IFVENDOR1, True, False
    )
    vlan = VlanDao.create_vlan(
        "192.168.1.1", 122, "192.168.1.1/22", "255.255.255.255", 1
    )
    assignment = AssignmentDao.create_assignment(
        "test", "test", "1234", 0, True, [""], cloud.name, vlan.vlan_id
    )
