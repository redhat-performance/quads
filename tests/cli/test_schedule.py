...
from tests.cli.config import CLOUD, HOST2, HOST1, DEFAULT_CLOUD, MOD_CLOUD, MODEL2
...
class TestSchedule(TestBase):
    ...
    def test_ls_schedule(self, remove_fixture):
        ...
        assert self._caplog.messages[2].startswith("6| start=")
        assert self._caplog.messages[3].startswith("8| start=")
    ...
    def test_ls_schedule_date(self, remove_fixture):
        ...
        assert self._caplog.messages[1] == f"Current cloud: {CLOUD}"
        assert self._caplog.messages[2].startswith("6| start=")
        assert self._caplog.messages[3].startswith("9| start=")
