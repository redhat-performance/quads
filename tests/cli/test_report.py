...
class TestReport(TestBase):
    ...
    def test_report_scheduled(self):
        ...
        assert (
            self._caplog.messages[0] == "Month   | Scheduled|  Systems|  % Utilized| "
        )
        assert (
            self._caplog.messages[1] == "2023-10 |         1|        2|         23%| "
        )
        assert (
            self._caplog.messages[2] == "2023-09 |         0|        2|          0%| "
        )
    ...
    def test_report_scheduled_year(self):
        ...
        assert (
            ...
            == f"{today.year}-{today.month:02d} |         1|        2|         23%| "
        )
        assert (
            self._caplog.messages[2]
            == f"{today.year}-{today.month - 1:02d} |         0|        2|          0%| "
        )
