...
class TestReadSchedule:
    ...
    def test_valid_current(self, test_client, auth, prefill):
        ...
        requests = [
            ...
            {"date": "2043-01-01"},
            {"cloud": schedule_responses[1][0]["host"]["cloud"]["name"], "date": "2050-01-01"}
        ]
...
class TestUpdateSchedule:
    ...
    def test_invalid_key_value_not_found(self, test_client, auth, prefill):
        ...
        assert response.status_code == 400
        assert response.json["error"] == "Bad Request"
        assert response.json["message"] == f"hostname {invalid_hostname} not found."
    ...
    def test_invalid_date_format(self, test_client, auth, prefill):
        ...
        assert response.status_code == 400
        assert response.json["error"] == "Bad Request"
        assert response.json["message"] == f"Invalid {date_type} date: {invalid_date}"
