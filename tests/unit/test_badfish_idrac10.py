...
OEM_OSD = "%s%s/Oem/Dell/DellOSDeploymentService" % (HOST_URI, SYSTEM_RESOURCE)
LEGACY_OSD = "%s/redfish/v1/Dell/Systems/System.Embedded.1/DellOSDeploymentService" % HOST_URI
...
class TestOsDeploymentResource:
    ...
    async def test_oem_resource(self, badfish_instance):
        badfish_instance.get_request = AsyncMock(side_effect=router({OEM_OSD: make_response()}))
    ...
    async def test_falls_back_to_legacy_resource(self, badfish_instance):
        badfish_instance.get_request = AsyncMock(side_effect=router({LEGACY_OSD: make_response()}))
    ...
    async def test_resources_follow_discovered_system_id(self, badfish_instance):
        ...
        legacy = "%s/redfish/v1/Dell/Systems/System.Embedded.2/DellOSDeploymentService" % HOST_URI
        badfish_instance.get_request = AsyncMock(side_effect=router({legacy: make_response()}))
    ...
    async def test_detach_remote_image_uses_resolved_resource(self, badfish_instance):
        ...
        expected = "%s/Actions/DellOSDeploymentService.DetachISOImage" % OEM_OSD
    ...
    async def test_check_remote_image_uses_resolved_resource(self, badfish_instance):
        ...
        expected = "%s/Actions/DellOSDeploymentService.GetAttachStatus" % OEM_OSD
        assert badfish_instance.post_request.call_args[0][0] == expected
