...
#### Ansible Dell Boot Order Playbooks
   - For Dell bare-metal systems we employ [optional boot interface order Ansible playbooks](https://github.com/redhat-performance/quads/tree/master/ansible), both for juggling boot interface order via racadm and for OpenStack deployments but also to provide setting interface boot order settings in a self-service fashion via automation with Foreman / Ansible.
