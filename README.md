...
If you really need to manually toggle individual cloud metadata settings this is possible as well, in this example we'll toggle the `validated` and `released` flags.

```
db.cloud.update({name:"cloud21"}, {$set:{validated:true}})
db.cloud.update({name:"cloud21"}, {$set:{released:true}})
```
