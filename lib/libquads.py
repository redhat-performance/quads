...
class Quads(object):
    ...
    def quads_write_data(self, doexit = True):
        try:
            ...
            stream.write( yaml.dump(self.quads.hosts.data, default_flow_style=False))
            stream.write( yaml.dump(self.quads.clouds.data, default_flow_style=False))
            stream.write( yaml.dump(self.quads.history.data, default_flow_style=False))
            if doexit:
                exit(0)
        except Exception, ex:
