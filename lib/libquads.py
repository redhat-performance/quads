...
class Hosts(object):
    def __init__(self, data):
        ...
        """
        self.logger = logging.getLogger("quads-logging.Hosts")
...
class Clouds(object):
    def __init__(self, data):
        ...
        data required by the Quads object.
        """
        self.logger = logging.getLogger("quads-logging.Clouds")
...
class Quads(object):
    def __init__(self, config, statedir, movecommand, datearg, syncstate, initialize, force):
        ...
        self.logger = logging.getLogger("quads-logging.Quads")
        ...
        for h in sorted(self.quads.hosts.data.iterkeys()):
            ...
            if not os.path.isfile(statedir + "/" + h):
                ...
                if current_state != current_cloud:
                    self.logger.info("INFO: Moving " + h + " from " + current_state + " to " + current_cloud)
                    if not dryrun:
                        try:
