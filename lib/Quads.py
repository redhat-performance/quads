...
class Quads(object):
    ...
    def read_data(self):
        if not os.path.isfile(self.config):
            data = {"clouds":{}, "powerstrips": {}, "hosts":{}, "history":{}, "cloud_history":{}}
            try:
    ...
    def write_data(self):
        if self.config_newer_than_data():
            ...
            try:
                self.data = {"clouds":self.quads.clouds.data,"powerstrips":self.quads.powerstrips.data, "hosts":self.quads.hosts.data, "history":self.quads.history.data, "cloud_history":self.quads.cloud_history.data}
                print self.data
    ...
    """
    """
    def get_outlet_brands(self):
        return Outlets.Outlets.get_brands()

    """
    """
    def add_powerstrip(self, brand, hostname, user, password):
        brands = self.get_outlet_brands()
        if brand not in brands :
            self.logger.error("Brand: {}, is not supported".format(brand))
            return ["ERROR"]
        if hostname not in self.quads.powerstrips.data:
            self.quads.powerstrips.hostname = {}
        self.quads.powerstrips.hostname = { "hostname": hostname,
                                            "user": user,
                                            "password": password,
                                            "last_update": "" }
        return ["OK"]
