...
class Quads(object):
    ...
    def read_data(self):
        if not os.path.isfile(self.config):
            data = {"clouds":{}, "outlets": {}, "hosts":{}, "history":{}, "cloud_history":{}}
            try:
    ...
    def write_data(self):
        if self.config_newer_than_data():
            ...
            try:
                self.data = {"clouds":self.quads.clouds.data,"outlets":self.quads.outlets.data, "hosts":self.quads.hosts.data, "history":self.quads.history.data, "cloud_history":self.quads.cloud_history.data}
                print self.data
    ...
    def update_host(self, hostresource, hostcloud, hosttype, forceupdate, outlet={hostname: None, port: None}):
        if outlet.hostname != None:
            if outlet.hostname not in self.quads.outlets :
    ...
    """
    """
    def add_outlets(self, brand, hostname, user, password):
        brands = self.get_outlet_brands()
        if brand not in brands :
            self.logger.error("Brand: {}, is not supported".format(brand))
            return ["ERROR"]
        if hostname not in self.quads.outlets.data:
            self.quads.outlets.hostname = { "hostname": hostname,
                                            "user": user,
                                            "password": password,
                                            "last_update": "" }
            return ["OK"]
