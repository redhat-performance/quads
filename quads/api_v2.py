...
from quads.model import Schedule, Host, Cloud, Interface, Assignment
                ...
                for _host in _hosts:
                    ...
                    try:
                        if _current_schedule:
                            _scheduled_cloud = _current_schedule.assignment.cloud.name
...
class DocumentMethodHandler(MethodHandlerBase):
    ...
    # currently only in use for clouds
    def POST(self, **data):
        # handle force
        ...
        obj_name = data["name"]
        result, obj_data = Assignment.prep_data(data)
        ...
        if result:
            ...
            try:
                ...
                if force and cloud_obj.assignment:
                    if cloud_obj.last_redefined:
                        cloud_reservation_lock = int(
                            conf["cloud_reservation_lock"]
                        )
                        lock_release = cloud_obj.last_redefined + datetime.timedelta(
                    ...
                    schedule_count = Schedule.objects(
                        assignment=cloud_obj.assignment, start__gte=datetime.datetime.now()
                    ).count()

                    current_schedule = Schedule.current_schedule(
                        assignment=cloud_obj.assignment
                    ).count()
                    if current_schedule:
                        if obj_data.get("wipe", False):
                            if obj_data["wipe"]:
                                obj_data.pop("wipe")
                    ...
                    cloud_obj.assignment.update(**obj_data)
                    cloud_obj.update(last_redefined=datetime.datetime.now())
                    cherrypy.response.status = "204 Resource updated"
                    result.append("Updated %s %s" % (self.name, obj_name))
                # otherwise create it
                else:
                    assignment = Assignment(**obj_data)
                    assignment.save()
                    cloud_obj.update(assignment=assignment, last_redefined=datetime.datetime.now())
...
class ScheduleMethodHandler(MethodHandlerBase):
    ...
    def POST(self, **data):
        ...
        if "cloud" in data:
            ...
            result.append("Missing argument: schedule-cloud")
            cherrypy.response.status = "400 Bad Request"
            return json.dumps({"result": result})
        ...
        if "index" in data:
            ...
            if schedule:
                ...
                if Schedule.is_host_available(
                    ...
                    assignment_obj.update(
                        notification__one_day=False,
                        notification__three_day=False,
                        notification__five_day=False,
                        notification__seven_day=False
                    )
            ...
            try:
                if Schedule.is_host_available(host=_host, start=_start, end=_end):
                    ...
                    if (
                            self.model.current_schedule(assignment=assignment_obj)
