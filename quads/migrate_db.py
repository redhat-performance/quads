...
def main():
    ...
    for host in hosts:
        with session_scope() as session:
            _cloud = session.query(models.Cloud).filter(name=host.cloud.name).first()
            _default_cloud = session.query(models.Cloud).filter(name=host.default_cloud.name).first()
            _host = models.Host(
                name=host.name,
                model=host.model,
                cloud=_cloud,
