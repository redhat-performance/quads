...
def switch_config(host, old_cloud, new_cloud):
    _host_obj = HostDao.get_host(host)
    _old_cloud_obj = CloudDao.get_cloud(old_cloud)
    _new_cloud_obj = CloudDao.get_cloud(new_cloud)
...
async def move_and_rebuild(host, new_cloud, semaphore, rebuild=False, loop=None):
    ...
    _target_cloud = CloudDao.get_cloud(new_cloud)
    ...
    if _target_cloud.name == _host_obj.default_cloud.name:
        ...
        source_cloud_schedule = ScheduleDao.get_current_schedule(cloud=_host_obj.cloud)
        if not source_cloud_schedule:
            _old_ass_cloud_obj = AssignmentDao.get_active_cloud_assignment(
                _host_obj.cloud
            )
            _old_ass_cloud_obj.active = False
            _old_ass_cloud_obj.vlan = None

    schedule = ScheduleDao.get_current_schedule(host=_host_obj, cloud=_target_cloud)
    if schedule:
        schedule.build_start = build_start
        schedule.build_end = datetime.now()

    logger.debug("Updating host: %s")
    _host_obj.cloud = _target_cloud
    _host_obj.build = False
    _host_obj.last_build = datetime.now()
    _host_obj.validated = False
    return True
