...
class Badfish:
    ...
    @staticmethod
    def error_handler(_response):
        try:
            data = _response.json(content_type="application/json")
        except ValueError:
    ...
    async def get_request(self, uri, _continue=False):
        try:
            async with self.session.get(uri, auth=BasicAuth(self.username, self.password), ssl=False, timeout=60) as _response:
