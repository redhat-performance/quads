...
class Populator(object):
    ...
    async def list_interfaces(badfish):
        data = None
        na_supported = await badfish.check_supported_network_interfaces("NetworkAdapters")
        ei_supported = await badfish.check_supported_network_interfaces("EthernetInterfaces")
        if na_supported:
        ...
        if interfaces:
            for interface, properties in interfaces.items():
                ...
                if mac_address:
                    ...
                    logger.error("  No MAC Address defined for this interface. SKIPPING.")
                    continue
                ...
                if values:
                    try:
                        ...
                        Host.objects.filter(name=host.name, interfaces__mac_address=values["mac_address"]).update_one(**kwargs)
                        logger.info(f"    Interface {mod_interface.name} successfully updated")
                    except Exception as ex:
                        logger.error(f"    Failed to update {mod_interface.name} interface")
                        logger.debug(ex)

        else:
            logger.warning(f"  No interfaces information could be retrieved for {host.name}")
