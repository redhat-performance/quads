...
@assignment_bp.route("/")
def get_assignments():
    _assignments = app.session.query(Assignment).all()
...
def get_assignment(assignment_id):
    _assignment = (
        app.session.query(Assignment).filter(Assignment.id == assignment_id).first()
    )
...
def create_assignment():
    ...
    if vlan:
        _vlan = app.session.query(Vlan).filter(Vlan.vlan_id == vlan).first()
        if not _vlan:
            return (
                jsonify(
                    {
    ...
    app.session.add(_assignment_obj)
    app.session.commit()
