...
class Host(Serialize, TimestampMixin, Base):
    ...
    # many-to-one
    cloud_id = Column(Integer, ForeignKey("clouds.id"))
    cloud = relationship("Cloud", foreign_keys=[cloud_id], cascade="all, delete")

    # many-to-one
    default_cloud_id = Column(Integer, ForeignKey("clouds.id"))
    default_cloud = relationship(
        "Cloud", foreign_keys=[default_cloud_id], cascade="all, delete"
    )

    # one-to-many parent
    interfaces = relationship("Interface", cascade="all, delete-orphan", lazy="joined")

    # one-to-many parent
    disks = relationship("Disk", cascade="all, delete-orphan", lazy="joined")

    # one-to-many parent
    memory = relationship("Memory", cascade="all, delete-orphan", lazy="joined")

    # one-to-many parent
    processors = relationship("Processor", cascade="all, delete-orphan", lazy="joined")
