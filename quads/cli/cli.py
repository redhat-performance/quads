...
class QuadsCli:
    ...
    def action_add_schedule(self):
        ...
        if self.cli_args["host"]:
            ...
            try:
                ...
                exit(1)
            status, result = loop.run_until_complete(jira.post_comment(_cloud.ticket, comment))
            if not status:
            ...
            for transition in transitions:
                ...
                if t_name and t_name.lower() == "scheduled":
                    ...
                    status, transition_result = loop.run_until_complete(
    ...
    def action_expirations(self):
        ...
        for cloud in clouds:
            ticket = cloud.ticket
            schedules = Schedule.objects(cloud=cloud, end__gte=today)
            extension = "None"
            if schedules:
                [status, issues] = loop.run_until_complete(jira.get_pending_child_extensions(ticket))
                if status and issues:
                    extension = issues[0].get("key")
                end = schedules[0].end
                days_left = end - today
                self.logger.info(f"{cloud.name}: {days_left.days} day[s], expires: {end.date()}, Extension: {extension}")
