...
from quads.config import QConfig as conf
...
class QuadsCli:
    ...
    def run(self, action: str, cli_args: dict) -> Optional[int]:
        ...
        if action:
            ...
            action_meth = getattr(self, self.ACTION_PREFIX + action, None)

            assert action_meth is not None and callable(
                action_meth
            ), f"Missing action method '{self.ACTION_PREFIX + action}', not implemented yet?"
        ...
        if self.cli_args.get("host_metadata") is not None:
            return self.action_host_metadata()
        ...
        # Add Schedule
        if self.cli_args.get("addschedule") is not None:
            return self.action_addschedule()
        ...
        if self.cli_args.get("mark_broken") is not None:
            return self.action_mark_broken()

        if self.cli_args.get("mark_repaired") is not None:
            return self.action_mark_repaired()

        if self.cli_args.get("retire") is not None:
            return self.action_retire()

        if self.cli_args.get("unretire") is not None:
            return self.action_unretire()
        ...
        if self.cli_args.get("summary") or self.cli_args.get("fullsummary"):
    ...
    def action_interface(self):
        ...
        if not host:
            raise CliException("Host {host} does not exist")

        data = self.quads.get_interfaces()
            ...
            for cloud in _clouds:
                ...
                if cloud.name == "cloud01":
                    if _kwargs.get("date"):
                        ...
                        try:
                            available_hosts = self.quads.get_available_hosts(**data)
...
>>>>>>> Stashed changes
    ...
    def action_rmcloud(self):
        ...
        self.output_json_result(_response, {"cloud": self.cli_args["rmcloud"]})
    ...
    def action_hostresource(self):
        ...
        self.output_json_result(_response, data)

    def action_host_metadata(self):
