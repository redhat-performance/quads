...
class QuadsCli:
    ...
    def run(self, action: str, cli_args: dict) -> Optional[int]:
        ...
        if self.cli_args.get("datearg"):
            _date = str(
                datetime.strptime(self.cli_args.get("datearg"), "%Y-%m-%d %H:%M")
            )
        for cloud in clouds:
    ...
    def action_extend(self):
        ...
        if not weeks and not date_arg:
            raise CliException("Missing option. Need --weeks or --date when using --extend")
        ...
        if weeks:
            ...
            end_date = datetime.strptime(date_arg, "%Y-%m-%d %H:%M")
            
        dispatch = {"cloud": self.quads.get_cloud, "host": self.quads.get_host}
        dispatch_key = "cloud" if cloud_name else "host"
        data = {"cloud": cloud_name} if cloud_name else {"host": host_name}
        
        try:
            dispatched_obj = dispatch[dispatch_key](data[dispatch_key])
            if not dispatched_obj:
            ...
            if not schedules:
                ...
                future_schedules = self.quads.get_future_schedules(data)
                if not future_schedules:
                    return

                if not self._confirmation_dialog(
                    f"Would you like to extend a future allocation of {data[dispatch_key]}? (y/N): "
                ):
                    return
                schedules = future_schedules
        except (APIServerException, APIBadRequest, CliException) as ex:
            raise CliException(str(ex))
        ...
        if not check:
            ...
            if weeks:
                self.logger.info(
                    f"{dispatch_key.capitalize()} {data[dispatch_key]} has now been extended for {str(weeks)} week[s] until {str(end_date)[:16]}"
                )
            else:
                self.logger.info(
                    f"{dispatch_key.capitalize()} {data[dispatch_key]} has now been extended until {str(_date)[:16]}"
                )
        else:
            self.logger.info(
                f"{dispatch_key.capitalize()} {data[dispatch_key]} can be extended until {str(end_date)[:16]}"
            )

    def action_shrink(self):
        if (
            not self.cli_args.get("weeks")
            and not self.cli_args.get("now")
            and not self.cli_args.get("datearg")
        ):
            raise CliException(
                "Missing option. Need --weeks, --date or --now when using --shrink"
            )

        if self.cli_args.get("cloud") is None and self.cli_args.get("host") is None:
            raise CliException(
        ...
        if self.cli_args.get("weeks"):
            try:
                weeks = int(self.cli_args.get("weeks"))
            except ValueError:
                raise CliException("The value of --weeks must be an integer")

            time_delta = timedelta(weeks=weeks)
        elif self.cli_args.get("datearg"):
            _date = datetime.strptime(self.cli_args.get("datearg"), "%Y-%m-%d %H:%M")
        elif self.cli_args.get("now"):
            _date = datetime.now()
        ...
        if self.cli_args.get("cloud"):
            ...
            for schedule in schedules:
                ...
                if (
                    ...
                    or (not self.cli_args.get("now") and end_date < threshold)
                ):
            ...
            if not self.cli_args.get("check"):
                confirm_msg = (
    ...
    def action_modcloud(self):
        ...
        try:
            response = self.quads.get_active_cloud_assignment(
                self.cli_args.get("cloud")
            )
        except (APIServerException, APIBadRequest) as ex:
            raise CliException(str(ex))
        ...
        for host_md in hosts_metadata:
            ...
            for key, value in host_md.items():
                if key != "name" and host[key]:
                    ...
                    if not self.cli_args.get("force"):
                        continue
                    if type(value) == list:
    ...
    def action_add_schedule(self):
        ...
        if self.cli_args.get("omitcloud"):
            ...
            omitted_cloud = [
                c for c in clouds if c.get("name") == self.cli_args.get("omitcloud")
            ]
        ...
        if self.cli_args.get("host"):
            if self.cli_args.get("omitcloud") and omitted_cloud_id:
                try:
                    host_obj = self.quads.get_host(self.cli_args.get("host"))
                except (APIServerException, APIBadRequest) as ex:
                    raise CliException(str(ex))
    ...
    def action_rminterface(self):
        ...
        data = {"host": self.cli_args.get("host"), "name": self.cli_args.get("ifname")}
        try:
