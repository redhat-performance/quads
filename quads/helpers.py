...
def get_vlan(cloud, index, last_nic=False):
    if cloud.assignment and cloud.assignment.vlan and last_nic:
        return int(cloud.assignment.vlan.vlan_id)
    else:
        vlan_first = int(conf.get("sw_vlan_first", 1100)) - 10
        cloud_offset = int(cloud.name[5:]) * 10
        base_vlan = vlan_first + cloud_offset
        if cloud.assignment and cloud.assignment.qinq == 1:
            index = 0
