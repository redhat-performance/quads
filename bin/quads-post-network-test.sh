...
scp $host_list $test_host:$host_list 1>/dev/null 2>&1

nic_test=$(ssh $test_host fping -u -f $host_list 2>&1 | grep -v "Warning: Permanently added")
...
fping -u \$(for h in \$(cat \$host_list) ; do host \$h ; done | awk '{ print \$NF }' | sed 's/10.12/172.16/')
EOF

if $firstnic ; then
    scp $tmp_script $test_host:$tmp_script 1>/dev/null 2>&1
    nic_test=$(ssh $test_host sh $tmp_script $host_list 2>&1 | grep -v "Warning: Permanently added")
