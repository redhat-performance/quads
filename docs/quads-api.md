...
```
curl -X POST http://localhost:8080/api/v2/cloudresource -d cloud=cloud02 -d description='New Updated Description' -d force=True -H 'Content-Type: application/json'
```

  - Add a new Host Schedule via API POST
```
curl -X POST http://localhost:8080/api/v2/addschedule -d host=host01.example.com -d 'schedstart=2019-09-01 22:00' -d 'schedend=2019-09-30 22:00' -d 'schedcloud=cloud02' -H 'Content-Type: application/json'
```
